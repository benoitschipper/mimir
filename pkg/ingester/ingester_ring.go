...
func (cfg *RingConfig) RegisterFlags(f *flag.FlagSet, logger log.Logger) {
	...
	f.DurationVar(&cfg.HeartbeatPeriod, prefix+"heartbeat-period", 2*time.Minute, "Period at which to heartbeat to the ring. 0 = disabled.")
	f.DurationVar(&cfg.HeartbeatTimeout, prefix+"heartbeat-timeout", 10*time.Minute, "The heartbeat timeout after which ingesters are skipped for reads/writes. 0 = never (timeout disabled)."+sharedOptionWithRingClient)
